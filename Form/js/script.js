...
function selectAll(checkbox){
	let all = document.getElementsByClassName("lineCheck");
...
function atualizaTabela() {
	...
	for (i = 0; i < localStorage.length; i++) {
		...
		if (dados != null) {
			...
			criaTd("<input class=\"lineCheck\" onclick=\"selectTd(this)\" type=\"checkbox\">", tr)
		}
	}
...
function editar() {
	...
	if (linhaSelecionada.length > 1) {
		...
		return;
	} else if(linhaSelecionada.length == 0){
		return;
	}
	allVarChange(true);
